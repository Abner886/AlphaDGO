...
def loss_complexity(loss_inputs, cfg):
    scores = loss_inputs.get('complexity_scores')
    if scores is None or scores.numel() == 0:
        device = scores.device if scores is not None else getattr(cfg, 'device', 'cpu')
        return torch.zeros((), device=device, dtype=torch.float32)
    mode = getattr(cfg, 'complexity_penalty_mode', 'mean')
    if mode == 'max':
        reward = scores.max()
    elif mode == 'median':
        reward = scores.median()
        if isinstance(reward, tuple):
            reward = reward[0]
    else:
        reward = scores.mean()
    # Negative sign turns higher scores into lower loss values so training favors logical factors.
    return -reward
...
def get_losses(loss_inputs,cfg):
    ...
    if getattr(cfg, 'l_complexity', 0.0) != 0:
        loss += cfg.l_complexity * loss_complexity(loss_inputs, cfg)
