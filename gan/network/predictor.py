...
class NetPMamba(nn.Module):
    ...
    def forward(self, x, latent: bool = False):
        x = x.float()
        x = self.input_proj(x)
        x = self.mamba(x)
        x = self.norm(x)
        x = x.mean(dim=1)
        latent_tensor = self.latent_proj(x)
        out = self.head(latent_tensor)
        if latent:
    ...
    def initialize_parameters(self):
        ...
        # for module in self.latent_proj:
...
class NetPLstm(nn.Module):
    ...
    def forward(self, x, latent: bool = False):
        ...
        latent_tensor = self.latent_proj(pooled)
        out = self.head(latent_tensor)
        if latent:
            return out, latent_tensor
        return out
